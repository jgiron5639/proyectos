...
<body>
  ...
  ?>
    ...
    <table>
      <tr>
      <h1 class="causaModificar">Editar Fichas</h1>
    ...
    $con=mysqli_connect("localhost", "root", "", "db_desercion");
    $id = $_GET["fic_numero"];
    $sql="SELECT * FROM ficha WHERE fic_numero = '$id'";
    $result=mysqli_query($con,$sql);
      while($row=mysqli_fetch_array($result)){
      ?>
      <tr>
        <input type="hidden" name="fic_numero" value="<?php echo $row['fic_numero']?>">
        <td class="tablaM"><input type="text" name="fic_numero" value="<?php echo $row['fic_numero']?>" readonly></td>
        <td class="tablaM"><select class="formfic_select" name="pro_codigo">
    <?php while($A = mysqli_fetch_array($programa)){ ?>

    <option value="<?php echo $A['pro_codigo'] ?>"><?php echo $A['pro_descripcion'] ?></option>
    ...
    ?>
        ...
        <td class="tablaM"><input type="date" name="fic_fecha_inicial" value="<?php echo $row['fic_fecha_inicial']?>"></td>
        <td class="tablaM"><input type="date" name="der_fecha_fin" value="<?php echo $row['der_fecha_fin']?>"></td>
        <td class="tablaM"><select class="formfic_select" name="cen_codigo">
    ...
    <a class="botonVolver" href="ficha.php">Volver</a>
    </form>
  </body>
</html>
