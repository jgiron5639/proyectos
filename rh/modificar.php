...
$sangre = mysqli_query($con, "SELECT rh_id , rh_descripcion FROM rh");
...
<body>
  <form class="formModificar" action="./modificar_validar.php" method="post">
    <table>
      <tr>
        <h1 class="causaModificar">Editar Tipo de Sangre</h1>
        <th>Codigo</th>
        <th>Nombre</th>
        </th>
      </tr>
      <?php
      $id = $_GET["rh_id"];
      $sql = "SELECT * FROM rh WHERE rh_id = '$id'";
      $result = mysqli_query($con,$sql);
      while ($row = mysqli_fetch_array($result)) {
      ?>
        <tr>
          <input type="hidden" name="rh_id" value="<?php echo $row['rh_id'] ?>">
          <td><input type="text" name="rh_id" value="<?php echo $row['rh_id'] ?>" readonly></td>
          <td><input type="text" name="rh_descripcion" value="<?php echo $row['rh_descripcion'] ?>"></td>

          <?php while ($tip = mysqli_fetch_array($sangre)) { ?>
    ...
    <a class="botonVolver" href="rh.php">Volver</a>
  </form>
</body>
