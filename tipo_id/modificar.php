...
$identificacion = mysqli_query($con, "SELECT tii_descripcion, tii_sigla FROM tipo_id");
...
<body>
  ...
  <body>
    <form class="formModificar" action="./modificar_validar.php" method="post">
      <table>
        <tr>
          <h1 class="causaModificar">Editar Tipo de Identificacion</h1>
          <th>Descripcion</th>
          <th>Siglas</th>
          </th>
        </tr>
        <?php
        $id = $_GET["tii_id"];
        $sql = "SELECT * FROM tipo_id WHERE tii_id = '$id'";
        $result = mysqli_query($con, $sql);
        while ($row = mysqli_fetch_array($result)) {
        ?>
          <tr>
            <input type="hidden" name="tii_id" value="<?php echo $row['tii_id'] ?>">
            <td><input type="text" name="tii_descripcion" value="<?php echo $row['tii_descripcion'] ?>"></td>
            <td><input type="text" name="tii_sigla" value="<?php echo $row['tii_sigla'] ?>"></td>
            <?php while ($iden = mysqli_fetch_array($identificacion)) { ?>
            <?php
            }
            ?>
      ...
      <a class="botonVolver" href="tipo_id.php">Volver</a>
    </form>
  </body>
