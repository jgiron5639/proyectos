...
$programa = mysqli_query($con, "SELECT pro_descripcion , pro_estado FROM programa");
...
<body>
  ...
  <body>
    <form class="formModificar" action="./modificar_validar.php" method="post">
      <table>
        <tr>
          <h1 class="causaModificar">Editar Programa</h1>
          <th>Codigo</th>
          <th>Nombre</th>
          <th>Estado</th>
          </th>
        ...
        <?php
        $id = $_GET["pro_codigo"];
        $sql = "SELECT * FROM programa WHERE pro_codigo = '$id'";
        $result = mysqli_query($con, $sql);
        while ($row = mysqli_fetch_array($result)) {
        ?>
          <tr>
            <input type="hidden" name="pro_codigo" value="<?php echo $row['pro_codigo'] ?>">
            <td><input type="text" name="pro_codigo" value="<?php echo $row['pro_codigo'] ?>" readonly></td>
            <td><input type="text" name="pro_descripcion" value="<?php echo $row['pro_descripcion'] ?>"></td>
            <td><input type="text" name="pro_estado" value="<?php echo $row['pro_estado'] ?>"></td>
            <?php while ($pro = mysqli_fetch_array($programa)) { ?>
            <?php
            }
            ?>
      ...
      <a class="botonVolver" href="programa.php">Volver</a>
    </form>
  </body>
