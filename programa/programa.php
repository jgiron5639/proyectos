...
<body>
  ...
  <!--/NAVBAR-->
  <form class="formPro" action="registrar_programa.php" method="POST">
    <h1>Registrar Programa</h1>
    <p>Descripción del Programa:</p>
    <input type="text" placeholder="Descripción" name="pro_descripcion" required>
    <select name="pro_estado" id="estado_id">
      <option value="activo">Activo</option>
      <option value="finalizado">Finalizado</option>
      <option value="cancelado">Cancelado</option>
    </select>
    <br>
    <button class="botonPro" name="enviar">Enviar</button><br>
    <br>
  </form>
  <?php

  include("../conexion.php");
  $program = "SELECT * FROM programa";
  ?>
  <h1 class="ListaPro">Listado Programa</h1>
  <div>
    <table class="tablaPro">
      <thead class="proHead">
      <tr class="BodyPro">
        <th class="tablaListaPro">Codigo</th>
        <th class="tablaListaPro">Nombre</th>
        <th class="tablaListaPro">Estado</th>
        <th class="tablaListaPro">Modificar</th>
        <th class="tablaListaPro">Eliminar</th>
      </tr>
      </thead>
      <?php $resultado = mysqli_query($con, $program);
      while ($row = mysqli_fetch_assoc($resultado)) { ?>
        <tr class="BodyPro">
          <td class="tablaListaPro"> <?php echo $row["pro_codigo"] ?></td>
          <td class="tablaListaPro"> <?php echo $row["pro_descripcion"] ?></td>
          <td class="tablaListaPro"> <?php echo $row["pro_estado"] ?></td>
          <td class="tablaListaPro"><a href="./modificar.php?pro_codigo=<?php echo $row['pro_codigo'] ?>"><button class="btn btn-outline-success" type="button" onclick="return ConfirmModf()">Modificar</button></a></td>
          <td class="tablaListaPro"><a href="./eliminar.php?pro_codigo=<?php echo $row['pro_codigo'] ?>"><button class="btn btn-outline-danger" type="button" onclick="return ConfirmDelete()">Eliminar</button></a></td>
        <?php }
