...
<body>
  ...
  if ($link) {
    mysqli_select_db($link, "db_desercion");
    mysqli_query($link, "SET NAMES 'utf8'");
  }
  ?>


  <form  class="formCiu" action="validar_registro.php" method="POST">
    <h1>Registro Ciudades</h1>
    <p>Codigo</p>
    <input  type="text" placeholder="Ingresa el codigo de la ciudad" name="ciu_id" required><br>
    <p>Nombre</p>
    <input  type="text" placeholder="Ingresa un nombre de ciudad" name="ciu_nombre" required><br>
    <p>Departamento</p>
    <select class="formCiu_select" name="dep_codigo"  required>
      <option value="">Departamento</option>
      <?php
      $v = mysqli_query($link, "SELECT * FROM departamento ");
      while ($departamento = mysqli_fetch_row($v)) {
      ?>
    ...
    <input type="number" placeholder="Ingresa cantidad de habitantes" name="ciu_cant_habitantes" required><br>
    <input class="botonCiu" type="submit" name="inscribir"><br>
  </form>
  ...
  $ciudad = "SELECT ciudad.ciu_id, ciudad.ciu_nombre, departamento.dep_codigo, departamento.dep_nombre, ciudad.ciu_cant_habitantes FROM ciudad INNER JOIN departamento on ciudad.dep_codigo=departamento.dep_codigo ";
  ?>
<h1 class="ListaCiu">Listado Ciudad</h1>
  <div>
    <table class="tablaCiudad">
      <thead class="ciuHead">
      <tr class="BodyCiu">
      <th class="tablaListaCiu"> Codigo</th>
        <th class="tablaListaCiu"> Nombre</th>
        <th class="tablaListaCiu"> Codigo Departamento</th>
        <th class="tablaListaCiu"> Departamento</th>
        <th class="tablaListaCiu"> Cantidad de Habit.</th>
        <th class="tablaListaCiu"> Modificar</th>
      </tr>
      </thead>
      <?php $resultado = mysqli_query($con, $ciudad);
      while ($row = mysqli_fetch_assoc($resultado)) { ?>
        <tr class="BodyCiu">
          <td class="tablaListaCiu"> <?php echo $row["ciu_id"] ?></td>
          <td class="tablaListaCiu"> <?php echo $row["ciu_nombre"] ?></td>
          <td class="tablaListaCiu"> <?php echo $row["dep_codigo"] ?></td>
          <td class="tablaListaCiu"> <?php echo $row["dep_nombre"] ?></td>
          <td class="tablaListaCiu"> <?php echo $row["ciu_cant_habitantes"] ?></td>
          <td class="tablaListaCiu"><a href="./modificar.php?ciu_id=<?php echo $row['ciu_id'] ?>"><button class="btn btn-outline-success" type="button" onclick="return ConfirmModf()">Modificar</button></a></td>
        <?php }
