...
$departamento = mysqli_query($con, "SELECT dep_codigo , dep_nombre FROM departamento");
...
<body>
  ...
  <body>
    ...
    <table>
      <tr>
      <h1 class="causaModificar">Editar Departamento</h1>
        <th>Codigo</th>
        <th>Nombre</th>
        </th>
      </tr>
    <?php
    $id = $_GET["dep_codigo"];
    $sql="SELECT * FROM departamento WHERE dep_codigo = '$id'";
    $result=mysqli_query($con,$sql);
      while($row=mysqli_fetch_array($result)){
      ?>
      <tr>
        <input type="hidden" name="dep_codigo" value="<?php echo $row['dep_codigo']?>">
        <td><input type="text" name="dep_codigo" value="<?php echo $row['dep_codigo']?>" readonly></td>
        <td><input type="text" name="dep_nombre" value="<?php echo $row['dep_nombre']?>"></td>
    ...
    <a class="botonVolver" href="departamento.php">Volver</a>
    </form>
  </body>
</html>